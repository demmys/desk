...
AttributeInfo *add_attribute_info(AttributeInfo *ais, u2 *count, AttributeTag tag, ...){
    ...
    switch(tag){
        ...
        case ATTRIBUTE_SourceFile:
            ai->u.cp_index = add_constant_info(CONSTANT_Utf8, va_arg(args, char *));
        ...
        case ATTRIBUTE_Exceptions:
            ea = &(ai->u.exceptions_attribute);
            ea->start_pc = va_arg(args, int);
            ea->end_pc = va_arg(args, int);
            ea->handler_pc = va_arg(args, int);
            ea->catch_type = va_arg(args, int);
            break;
